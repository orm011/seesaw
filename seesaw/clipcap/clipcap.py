...
class Captioner2:
    def __init__(self, device, clip_cap_root, mode="tx"):
        clip_model, preprocess = clip.load(
            "RN50x4", device=device, download_root=f"{clip_cap_root}/clip/", jit=False
        )
        ...
        tokenizer = GPT2Tokenizer.from_pretrained(
            f"{clip_cap_root}/gpt2/tokenizer/", local_files_only=True
        )
        model_path = f"{clip_cap_root}/CLIPcap_model_weights/transformer_weights.pt"
        self.prefix_length = 40
        self.tokenizer = tokenizer
        self.gpt = GPT2LMHeadModel.from_pretrained(f"{clip_cap_root}/gpt2/lmhead")
    ...
    def caption(self, pil_image, use_beam_search=False):

        prefix_embed = self._prefix(pil_image)
...
class Captioner:
    def __init__(self, device, clip_cap_root, mode="tx"):
        assert mode in ["tx", "coco", "conceptual"]
        self.mode = mode
        clip_variant = "RN50x4" if mode == "tx" else "ViT-B/32"
        self.prefix_length = 40 if mode == "tx" else 10
        self.model_weights = (
            "transformer_weights"
            if mode == "tx"
            else "coco_weights"
            if mode == "coco"
            else "conceptual_weights"
        )
        ...
        clip_model, preprocess = clip.load(
            ...
            device=device,
            jit=False,
            download_root=f"{clip_cap_root}/clip/",
        )
        self.device = device
        self.clip_model = clip_model
        self.preprocess = preprocess
        ...
        model_path = f"{clip_cap_root}/CLIPcap_model_weights/{self.model_weights}.pt"
        self.tokenizer = tokenizer
        self.gpt = GPT2LMHeadModel.from_pretrained(f"{clip_cap_root}/gpt2/lmhead")

        if self.mode != "tx":
            model = ClipCaptionModel(self.prefix_length, gpt=self.gpt)
        else:
            model = ClipCaptionPrefix(
                prefix_length=self.prefix_length,
                clip_length=40,
