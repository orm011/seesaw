...
from bokeh.models import HoverTool, ColumnDataSource, CategoricalColorMapper, CategoricalScale, CustomJS, Button
...
def fit_projection(self):
    ...
    self.projected_images = reducer.transform(sklearn.preprocessing.normalize(db.embedded))
    self.projected_text = reducer.transform(self.query_vectors)

    db_df = pd.DataFrame(self.projected_images, columns=('x', 'y'))
    self.df_projected_images = db_df.assign(point_id = np.arange(len(self.ev)).astype('int'), image_url=ev.db.urls)
    self.df_projected_text = queries.assign(x=self.projected_text[:,0], y=self.projected_text[:,1])
        
def assign_clusters(self):
    hdb = hdbscan.HDBSCAN(min_cluster_size=40)
    hdb.fit(self.projected_images)
    self.hdb = hdb
    self.df_projected_images = self.df_projected_images.assign(cluster_id=[i for i in hdb.labels_])
...
def assign_top_scores(self, topk):
    ...
    for (qstr,vc) in tqdm(list(zip(self.queries.query_str, self.query_vectors))):
        a,b = self.ev.db.query(topk=topk, mode='nearest', vector=vc.reshape(1,-1), return_scores=True)
        topvecs[qstr] = a
        topscores[qstr] = b
...
def make_db_plot(vz, show_text=False):
    ...
    db_scatter = plot_figure.circle(x='x', y='y', radius=.01, radius_units='data', fill_alpha=.5,
    line_alpha=.5, color=dict(field='cluster_id', transform=color_mapping), source=db_cds)
    ...
    segment_source = ColumnDataSource({'x0': [], 'y0': [], 'x1': [], 'y1': [], 'color':[], 'width':[], 'rank':[],'image_url':[],
                                      'score':[]})
    neighbor_segment = plot_figure.segment(x0='x0', y0='y0', x1='x1', y1='y1', color={'field':'color'}, 
                        alpha=0.5, line_width={'field':'width'}, source=segment_source)
    ...
    plot_figure.scatter(x='x', y='y', color='black', size=10, alpha=.9, 
                        fill_alpha=0., marker=dict(value='cross'),
                       source=gt_source)
    ...
    if show_text:        
        ...
        select_cb = CustomJS(args=dict(lang_cds=lang_cds,
                                db_cds = db_cds,
                                gt_cds = gt_cds,
                                db_top=db_top,
                                db_score=db_score,
...
def animate_trace(vz, fig_fun, sample_trace, ground_truth):
    ...
    search_cds = ColumnDataSource({'x':[], 'y':[], 'color':[], 'alpha':[], 'width':[], 'batch':[]})
    db_scatter = plot_figure.scatter(x='x', y='y', color=dict(field='color'), size=10, line_width=dict(field='width'), alpha=dict(field='alpha'), 
                        fill_alpha=0., marker=dict(value='circle'),
                       source=search_cds)
    plot_figure.add_tools(HoverTool(tooltips="""
    <div>
    ...
    button.js_on_click(CustomJS(args=dict(search_cds=search_cds, 
                                          db_df=ColumnDataSource(vz.df_projected_images),
                                          gt_source=gt_source,
                                          iter=[0],
                                          ground_truth=ground_truth,
                                          selection=np.where(ground_truth > 0)[0],
                                          sample_trace=sample_trace, interval_ms=1000), code="""
                                    // clear previous data
